...
export default function LoginPage() {
  ...
  // Helper to safely get translation key with fallback
  const getTranslation = (key) => {
    const langObj = translations[selectedLanguage] || translations['English'];
  ...
  return (
    ...
    >
      ...
      <div style={{
        background: 'linear-gradient(135deg, #0A1A2F 0%, #0F2C3F 40%, #1A3E52 70%, #0A1A2F 100%)',
        opacity: 0.95
      }} className="w-full max-w-md mx-4 rounded-2xl shadow-lg flex flex-col items-center">
        ...
        {form === "login" && (
          <form onSubmit={handleLogin} className="space-y-6 w-full">
            <div className="relative mb-4">
              <label className="block text-white mb-1">{getTranslation('emailOrUsername')}</label>
              <input
                type="text"
                placeholder={getTranslation('emailOrUsername')}
                className="w-full border-b border-gray-200 bg-transparent py-3 pl-10 pr-4 text-white placeholder-gray-200 focus:outline-none focus:border-[#1A3E52]"
                value={email}
            ...
            <div className="relative mb-6">
              <label className="block text-white mb-1">{getTranslation('password')}</label>
              <input
                type="password"
            ...
            {error && <div className="text-[#1A3E52] text-sm text-center">{error}</div>}
            <button
              type="submit"
              style={{background: 'linear-gradient(135deg, #E8FBFF 0%, #3EC7FF 30%, #0AA1D9 65%, #042C57 100%)'}}
          ...
          <form onSubmit={handleRegister} className="space-y-5">
            <div className="flex gap-2">
              <div className="flex-1">
                <label className="block text-white mb-1">{getTranslation('firstName')}</label>
                <input
                  type="text"
                  className="w-full border-b-2 text-white border-gray-300 focus:border-[#1A3E52] bg-transparent py-2 pl-2 outline-none"
                  value={regFirstName}
              ...
              <div className="flex-1">
                <label className="block text-white mb-1">{getTranslation('lastName')}</label>
                <input
                  type="text"
                  className="w-full border-b-2 text-white border-gray-300 focus:border-[#1A3E52] bg-transparent py-2 pl-2 outline-none"
                  value={regLastName}
            ...
            <div>
                <label className="block text-white mb-1">{getTranslation('email')}</label>
              <input
                type="email"
                className="w-full border-b-2 text-white border-gray-300 focus:border-[#1A3E52] bg-transparent py-2 pl-2 outline-none"
                value={regEmail}
        ...
        {form === "forgot" && (
          <form onSubmit={handleForgot} className="space-y-5">
            {forgotStep === 1 && (
              <>
                <div>
                  ...
                  <input
                    type="email"
                    className="w-full text-white border-b-2 border-gray-300 focus:border-[#1A3E52] bg-transparent py-2 pl-2 outline-none"
                    value={forgotEmail}
            ...
            {forgotStep === 2 && (
              <>
                <div>
                  <label className="block text-white mb-1">{getTranslation('newPassword')}</label>
                  <input
                    type="password"
                    className="w-full text-white border-b-2 border-gray-300 focus:border-[#1A3E52] bg-transparent py-2 pl-2 outline-none"
                    value={newPassword}
                ...
                <div>
                  <label className="block text-white mb-1">{getTranslation('confirmPassword')}</label>
                  <input
                    type="password"
                    className="w-full text-white border-b-2 border-gray-300 focus:border-[#1A3E52] bg-transparent py-2 pl-2 outline-none"
